...
exports.signup = async (req, res) => {
  const cipherEmail = CryptoJS.AES.encrypt(req.body.email, process.env.CIPHER).toString();
  User.findOne(cipherEmail,req.body.pseudo, function(err, results, fields) {
    ...
    if (results.length > 0) {
      ...
      .then(hash =>{
          User.createOne(req.body.pseudo, req.body.firstname, req.body.lastname, cipherEmail, hash, function(err, result, fields) {
            if(err) {    return res.status(500).send({ error: "Erreur serveur => " + err });   }
            const tokenObject = await token.issueToken(result[0]);
...
exports.login = async (req, res) => {
  const cipherEmail = CryptoJS.AES.encrypt(req.body.contact.email, process.env.CIPHER).toString();
