    ...
    <div class="onePost">
         <div class="post-wrapper" v-if="!modify">
            ...
            <div class="post-infos">
                <span class="post-info">Posté le {{dateFormat(post.creationDate)}} par
                    <router-link :to="{ name: 'User', params: { id: post.userId } }"> {{showPseudo(post)}}
                        </router-link> 
                    </span>
                ...
                <div>
                    <div> 
                        ...
                        <button v-if="comments.length != 0" @click="showComments = !showComments"><span v-if="!showComments">Voir</span><span v-else>Cacher</span> le<span v-if="comments.length != 1">s</span> commentaire<span v-if="comments.length != 1">s</span></button>
                    </div> 
        ...
        <p>{{errorMessage}}{{messageRetour}}</p>
        ...
        <button v-if="modify" @click="modifyOnePost(this.post.id)">Publier les modifications</button>
        <button v-if="post.userId == user.userId || user.admin == 1"  @click.prevent="deletePost(post.id)">Supprimer le post</button>

        <div class="comments-wrapper" v-if="!modify && showComments">
...
export default {
    ...
    methods: {
        ...
        modifyOnePost(postId){
            ...
            if (this.withImage && this.file) { formData.set("image", this.file); }
